...
def draw_boxes(img, boxes, scale):
    ...
    for box in boxes:
        ...
        cv2.line(img, (int(box[0]), int(box[1])), (int(box[0]), int(box[5])), color, 2)
        cv2.line(img, (int(box[2]), int(box[5])), (int(box[2]), int(box[1])), color, 2)
        cv2.line(img, (int(box[0]), int(box[5])), (int(box[2]), int(box[5])), color, 2)
    ...
    return text_recs, img, scale


def text_detect(img):
    scores, boxes, img, scale = ctpn(img)
    text_recs, img_drawed, scale = draw_boxes(img, boxes, scale)
